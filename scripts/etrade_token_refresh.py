    ...
    with psycopg.connect(dsn) as conn:
        ...
        if renew_only:
            print(
                ...
                "Connect in the Streamlit app (writes etrade_sessions for GitHub scans).",
                file=sys.stderr,
            )
            return 1
        ...
        if _in_github_actions() and not _force_browser():
            if existing:
                print(
                    ...
                    "Mint a token in the app after midnight ET.",
                    file=sys.stderr,
                )
                return 0
            print(
                "No etrade_sessions tokens. Connect in the Streamlit app first.",
                file=sys.stderr,
            )
            return 1
